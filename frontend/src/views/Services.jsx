...
import { Modal, ConfirmDialog } from '../components.jsx'
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start justify-between gap-4">
        ...
        <button type="button" className="btn btn-primary" data-testid="service-open" onClick={() => setDialogOpen(true)}>Register service</button>
      </div>
      {msg && (
        <div className={`text-sm ${msg.ok ? 'text-emerald-400' : 'text-rose-400'}`} data-testid="service-result">{msg.text}</div>
      )}
      <Modal open={dialogOpen} title="Service registry entry" description="Backends are proxied server-side; product frontends stay internal only." onClose={() => setDialogOpen(false)}>
      ...
      <ConfirmDialog
        ...
        message={`Remove service "${form.id}" from the registry? The Hub stops proxying it immediately.`}
